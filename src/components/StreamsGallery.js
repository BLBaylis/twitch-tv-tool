...
class StreamsGallery extends React.Component {
	...
	constructStreams(data, online) {
		...
		let streams = data.map(x => <StreamCard filter = {this.state.filter.toLowerCase()} search = {""} twitch = {this.props.currentData === "Twitch"} key = {x.streamName} data = {x} online = {x.online}/>);
		return streams;
	}
		...
		return (
			<div className = "streams-gallery">
				...
				<div className = "streams-gallery--streams">
					{(!twitchTest && this.state.filter === "All") && <div className = "streams--stream__all">
						{this.constructStreams(this.props.data, true)}
						{this.constructStreams(this.props.data, false)}
					</div>}
				{(this.state.filter === "Online" || twitchTest) && <div className = "streams--stream__online">
					{this.constructStreams(this.props.data, true)}
				</div>}
					{this.state.filter === "Offline" && <div className = "streams--stream__offline">
						{this.constructStreams(this.props.data, false)}
					</div>}
				</div>
			</div>
