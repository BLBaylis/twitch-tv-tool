...
class App extends React.Component {
  ...
  tabsHandler(event) {
    ...
    document
      .getElementsByClassName("tab-navbar--tab__active")[0]
      .classList.remove("tab-navbar--tab__active");
  ...
  getAllNonTwitchData(recommendedBy) {
    for (let i = 0; i < this.state.streams[recommendedBy].length; i++) {
      this.getNonTwitchData(
        this.state.streams[recommendedBy][i],
        recommendedBy
      );
    }
  }
  ...
  async getAllData() {
    ...
    this.setState({ refreshSpin: false });
  }
      ...
      <div className="app">
        <header className="app--header">
          <h1 className="app--title">
            Brad's{" "}
            <img src={twitch} className="app--title__logo" alt="Twitch logo" />{" "}
            Tool
          </h1>
        </header>
        <StreamerSearch />
        <RefreshBtn
          refresh={this.dataThrottle}
          refreshSpin={this.state.refreshSpin}
        />
